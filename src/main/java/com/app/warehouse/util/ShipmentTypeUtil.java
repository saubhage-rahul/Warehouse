...
public class ShipmentTypeUtil {
	...
	private static final Logger log = LoggerFactory.getLogger(ShipmentTypeUtil.class);
	...
	public void generateFreeChart(String path, List<Object[]> data) {
		...
		// Input => title, dataset
		JFreeChart chart = ChartFactory.createPieChart("ShipmentTypeMode", dataset);

		// 3. save as Image
		try {
			ChartUtils.saveChartAsJPEG(new File(path + "/shipmentTypeMode.jpg"), // file location + name
